...
type BroadcastMessage struct {
	...
	Portfolio []models.PortfolioEntry `json:"portfolio,omitempty"`
...
func (h *Hub) Run(ctx context.Context) {
	...
	for {
		...
		case msg := <-h.broadcastSignal:
			...
			if msg.IsActive {
				err = h.store.UpdateStore(ctx, tick)
...
func (c *Client) WritePump() {
	defer func() {
		if r := recover(); r != nil {
			logs.Log.Warn("WritePump recovered from panic (client likely disconnected abruptly)", zap.Any("recover", r))
		}
		c.Conn.Close()
	}()
