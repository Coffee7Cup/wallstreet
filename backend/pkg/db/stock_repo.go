...
// --- connection-scoped helpers for UpdateStore ---
// These run queries on a pre-acquired *pgxpool.Conn so the entire fundamentals
// refresh loop shares a single connection instead of borrowing one per query.

func (s *Store) getRatiosConn(ctx context.Context, conn *pgxpool.Conn, companyID int, date time.Time) ([]models.Ratio, error) {
	rows, err := conn.Query(ctx, "SELECT id, company_id, year, COALESCE(opm, 0), COALESCE(debtor_days, 0), COALESCE(inventory_days, 0), COALESCE(days_payable, 0), COALESCE(cash_conversion_cycle, 0), COALESCE(working_capital_days, 0), COALESCE(roce_percent, 0) FROM ratios WHERE company_id = $1 AND EXTRACT(YEAR FROM year) <= $2 ORDER BY year DESC", companyID, date.Year())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var r models.Ratio
		if err := rows.Scan(&r.ID, &r.CompanyID, &r.Year, &r.OPM, &r.DebtorDays, &r.InventoryDays, &r.DaysPayable, &r.CashConversionCycle, &r.WorkingCapitalDays, &r.ROCEPercent); err != nil {
			return nil, err
		}
		ratios = append(ratios, r)
	}
...
func (s *Store) getProfitLossConn(ctx context.Context, conn *pgxpool.Conn, companyID int, date time.Time) ([]models.ProfitLoss, error) {
...
func (s *Store) getBalanceSheetsConn(ctx context.Context, conn *pgxpool.Conn, companyID int, date time.Time) ([]models.BalanceSheet, error) {
	rows, err := conn.Query(ctx, `
		SELECT id, company_id, year, COALESCE(equity_capital, 0), COALESCE(reserves, 0), COALESCE(borrowings, 0), COALESCE(other_liabilities, 0), COALESCE(total_liabilities, 0), COALESCE(fixed_assets, 0), COALESCE(cwip, 0), COALESCE(investments, 0), COALESCE(other_assets, 0), COALESCE(total_assets, 0)
		FROM balance_sheets
		WHERE company_id = $1 AND EXTRACT(YEAR FROM year) <= $2
...
func (s *Store) getCashFlowsConn(ctx context.Context, conn *pgxpool.Conn, companyID int, date time.Time) ([]models.CashFlow, error) {
	rows, err := conn.Query(ctx, `
		SELECT id, company_id, year, COALESCE(cash_from_operating_activity, 0), COALESCE(cash_from_investing_activity, 0), COALESCE(cash_from_financing_activity, 0), COALESCE(net_cash_flow, 0)
		FROM cash_flows
		WHERE company_id = $1 AND EXTRACT(YEAR FROM year) <= $2
