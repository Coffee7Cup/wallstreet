...
func (s *Store) GetUserByID(ctx context.Context, id int) (*models.User, error) {
	...
	if err != nil {
		logs.Log.Error("Failed to get user by ID", zap.Int("id", id), zap.Error(err))
		return nil, err
	}
...
func (s *Store) GetUserPortfolio(ctx context.Context, userId int, tick int) ([]models.PortfolioEntry, error) {
	rows, err := s.pool.Query(
		ctx,
		`SELECT p.user_id, p.company_id, c.name, c.symbol, p.quantity, COALESCE(s.close_price, 0)
		 FROM portfolio_entries p
		 JOIN companies c ON p.company_id = c.id
		 LEFT JOIN stock_prices_with_ticks s ON p.company_id = s.company_id AND s.tick_idx = $2
		 WHERE p.user_id = $1`,
		userId,
		tick,
	)
	...
	if err != nil {
		logs.Log.Error("Failed to query user portfolio", zap.Int("user_id", userId), zap.Int("tick", tick), zap.Error(err))
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.UserID, &p.CompanyID, &p.CompanyName, &p.CompanySymbol, &p.Quantity, &p.CurrentPrice); err != nil {
