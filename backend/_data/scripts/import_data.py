...
def process_table(cur, csv_path, temp_table, main_table, company_symbol, cols):
    ...
    conflict_cols = (
        "company_id, date" if main_table == "stock_prices" else "company_id, year"
    )

    update_clause = ",\n".join(
        [f"{c} = EXCLUDED.{c}" for c in cols if c not in ("year", "date")]
    )
    ...
    for sector_dir in DATA_DIR.iterdir():
        ...
        for company_dir in sector_dir.iterdir():
            ...
            if prices_csv.exists():
                with open(prices_csv, "r", encoding="utf-8-sig") as f:
                    ...
                    for row in reader:
                        ...
                        data.append(
                            (
                                date_val,
                                clean(row.get("Open Price", "0")),
                                clean(row.get("High Price", "0")),
                                clean(row.get("Low Price", "0")),
                                clean(row.get("Close Price", "0")),
                                shares,
                                trades,
                                turnover,
                            )
                ...
                if data:
                    cur.execute(
                        "CREATE TEMP TABLE raw_prices (date DATE, open_price NUMERIC, high_price NUMERIC, low_price NUMERIC, close_price NUMERIC, no_of_shares NUMERIC, no_of_trades NUMERIC, total_turnover NUMERIC)"
                    )
            ...
            process_table(
                cur,
                company_dir / "balance_sheets.csv",
                "balance_sheets_temp",
                "balance_sheets",
                symbol,
                bs_cols,
            ...
            process_table(
                cur, company_dir / "ratios.csv", "ratios_temp", "ratios", symbol, r_cols
            )
